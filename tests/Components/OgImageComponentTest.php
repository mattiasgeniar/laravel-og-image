...
it('renders template tag from blade component', function () {
    $view = $this->blade('<x-og-image><div>Hello World</div></x-og-image>');

    $hash = md5('<div>Hello World</div>');

    $view->assertSee('<template data-og-image', false);
    $view->assertSee("data-og-hash=\"{$hash}\"", false);
    $view->assertSee('data-og-format="jpeg"', false);
    $view->assertSee('</template>', false);
});

it('does not include meta tags inline', function () {
    $view = $this->blade('<x-og-image><div>Hello World</div></x-og-image>');

    $view->assertDontSee('<meta property="og:image"', false);
    $view->assertDontSee('<meta name="twitter:image"', false);
});
...
it('includes the slot content inside the template tag', function () {
    ...
    $view->assertSee('<div class="my-og-content">Hello</div>', false);
    $view->assertSee('data-og-image', false);
});
...
it('accepts a format attribute', function () {
    ...
    $view->assertSee('data-og-format="webp"', false);
});
